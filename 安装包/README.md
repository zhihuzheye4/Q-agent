...
| v0.0.19 | 2026-06-30 | _pending_ | **工具层实化（v0.0.19 工具层全生命周期，UI 仍看不到变化——v0.0.20 接通编排层才触发）**：8 个 M 里程碑全完成——① **M1 协议层**：`spec.py` 新建 ToolSpec dataclass + ExternalToolSource Protocol（runtime_checkable，为 v0.0.21+ MCP 铺路）；`base.py` ToolMeta 加 6 字段（version/timeout/long_running/concurrency_safe/permission_level/needs_confirmation）+ to_spec() 转换；`registry.py` @tool 装饰器签名扩展 6 参数 + needs_confirmation=None 自动推导 + lookup_spec()。② **M2 安全层扩展**：`safety.py` 加 4 类校验——check_sensitive（fnmatch 匹配 .env/*.pem/id_rsa/credentials 等 + 用户自定义 ~/.q-agent/sensitive.txt）+ sniff_secret_content（PEM/AKIA/ghp_/sk- 正则）+ check_url（强制 HTTPS + 域名白名单 pypi/github/huggingface/docs.python.org/localhost:11434）+ check_ssrf（私网段 10.x/172.16-31.x/192.168.x/127.x/169.254.x/::1/fc00::/7 拦截）。③ **M7 撤销栈**（先于 M3 做）：`undo.py` snapshot_before_write/move + undo_last + 内存栈容量 20 + 备份到 ~/.q-agent/snapshots/。④ **M3 工具实装层 10 工具按 4 文件**：`_helpers.py` error_json + html_to_markdown 零依赖；`files.py` 5 工具（file_read/write/edit/list_dir/move，file_edit 是讨论后追加的 P0 工具精确字符串替换）；`search.py` search_content（正则 + 500KB 文件跳过）+ search_glob；`exec.py` exec_shell（destructive + needs_confirmation=True，long_running=True→600s）；`web.py` web_get/web_fetch（urllib + html.parser 零依赖）。⑤ **M4 持久化层**：`persistence.py` SCHEMA_SQL 追加 tool_calls + tool_audit 两表 + 5 方法（含 load_all_tool_calls 供工具历史窗口用）；`audit.py` args_hash + summarize + write_audit；`results.py` 落盘 .txt + .meta.json + 30 天清理；`budget.py` apply_budget（L1 占位）+ truncate（L2 头尾保留）+ summarize_placeholder（L3 v0.0.20 接 SummaryWorker）。⑥ **M5 审批与权限**：`approval.py` 三级权限判定（read_only 自动通过 / write 首次弹窗后 args_hash 缓存 / destructive 永不缓存）+ deny 缓存 + needs_sandbox_risk_ack/ack_sandbox_risk（~/.q-agent/settings.json）；`approval_dialog.py` ApprovalDialog（approved 信号 always/once/deny + QCheckBox，destructive 禁用 cb）+ SandboxRiskDialog（永不沙箱风险知情一次性弹窗，未勾选时确认按钮禁用）。⑦ **M6 UI 反馈**：`tool_bubble.py` ToolBubble(QFrame) 4 状态 pending/success/failed/cancelled + 折叠 28px / 展开 240px + QPropertyAnimation 200ms OutCubic；`tool_history_window.py` 独立 QMainWindow 6 列从 sqlite 重建；`chat_page.py` 最小侵入挂载（外层 layout addWidget，不动 messages_layout 避免 _clear_messages 冲突，信号连接注释掉 v0.0.20 取消注释）；`main_window.py` 菜单栏追加"工具 → 工具历史"Ctrl+H；`scripts/generate_icons.py` 新增 4 工具状态图标定义 + 4 SVG 生成 + manifest.json 更新。⑧ **M8 打包发布**：升版本 0.0.18→0.0.19 + PyInstaller 按 README.md 标准模板打 v0.0.19.exe（删 v0.0.18 子目录）+ 版本历史追加。测试 408 通过（+157 新增：14 spec + 41 safety + 50 工具 + 15 持久化 + 8 undo + 21 approval + 8 tool_bubble），ruff/mypy strict 全绿。落实 ADR-027 模块化原则的第六个执行案例——工具层完全独立挂载，编排层未接通前所有 widget/弹窗建好但不触发。下一步 v0.0.20 接通编排层。 |