...
确立**模块化开发原则**作为本项目 UI 层的核心架构规则：

1. **既有模块与新模块的划分**：
   - 既有模块 = v0.0.1 写好的 UI 框架（MainWindow 布局骨架、Sidebar 4 tab、ChatPage、Toolbar、MenuBar、theme QSS、icons 资源系统）。既有模块写好后永不再动（除非明确扩展公开接口）。
   - 新模块 = 每个新功能 = 独立 widget 文件 + 父容器一行 `addWidget` 挂载 + 一行信号槽连接。
...
- **模块化 vs 容器化内嵌**：容器化看似"紧凑"，但破坏既有模块内聚性，导致 QSS/事件/资源连锁失配。模块化牺牲一点布局嵌套（多一层 left panel），换既有模块零侵入，长期可维护性远胜。
...
- 既有模块本身的演进（v0.0.1 UI 框架如有结构性缺陷，仍可通过展示方案后重构，本原则只约束"新功能增加"路径）
- 非 UI 层的模块化（编排/记忆/技能层是否同样适用，等用户提需求时再讨论）

### 关联

- 扩展 ADR-005（项目框架搭建策略）：既有模块 = 框架阶段一次性做好的部分
- 扩展第十九节（指令执行自由度）：明文化"架构级改动"的具体判定
- 被本 ADR 修正的历史决策：ADR-026 中"Sidebar 从 QListWidget 改为 QFrame 容器"的部分被 ADR-028 回退

---

## ADR-028：v0.0.12 sidebar 容器化违反模块化原则的回退决策（v0.0.14）
...
v0.0.12 ADR-026 决策"Sidebar 从 QListWidget 改为 QFrame 容器（含 QListWidget 上半部分 tab 列表 + HardwareMonitor 下半部分折线图）"违反 ADR-027 模块化开发原则，连锁引发 4 个问题（详见 ADR-027 背景段）。
...
2. **main_window.py 新建 left panel 模块化挂载**：
...
4. **hardware_monitor.py 保持不变**（已是独立模块，符合模块化）
...
- v0.0.14 起，sidebar 回到 v0.0.9 稳定版本，后续新功能不得再侵入
...
5. **hardware_monitor.py 拆分**：只保留 Worker + 常量 + METRICS + collect_sample_sync（测试辅助）；HardwareMonitorWindow + MonitorCell 独立到 hardware_monitor_window.py，符合模块化"独立 widget 文件"原则。
...
- 落实 ADR-027 模块化开发原则的第二个执行案例（首个为 v0.0.14 left panel 一行挂载）
- 与 ADR-024（加载指示器）共享"独立 widget 文件"模块化模式
...
- Toolbar 类继承/objectName/既有信号全不变，仅新增 2 信号——符合模块化原则
...
- 落实 ADR-027 模块化原则的第三个执行案例（v0.0.14 left panel 挂载 + v0.0.15 独立窗口 + v0.0.16 toolbar 信号扩展）
...
## ADR-031：取消按钮接通 ChatWorker.stop()（接口预留型待办第二例）

**日期**：2026-06-21
**状态**：已实施（v0.0.17 候选 1，feat/cancel-button 分支待用户验证）
**取代**：无
**关联**：ADR-22（ChatWorker 流式批量刷新，v0.0.8 已留 stop() 占位）、ADR-27（模块化开发原则）、ADR-30（接口预留型待办首例 v0.0.8 status_callback → v0.0.16）

### 背景

v0.0.8 实现 ChatWorker 时已预留 stop() 方法（设 _stop 标志）+ chat_aborted 信号占位，toolbar"取消"按钮 triggered 连到 status_callback 占位文案。用户在 v0.0.16 完成后选择执行候选 1（取消按钮接通 ChatWorker.stop）。

### 决策

- toolbar.py 扩展公开接口（纯增量）：新增 cancel_requested = Signal() 信号；_build_actions 加 cancel-action（stop 图标，"取消当前 AI 回复生成"tooltip），triggered 连 cancel_requested.emit。不改类继承/objectName/既有信号。
- chat_worker.py 扩展公开接口（纯增量）：新增 chat_aborted = Signal()；run() 在 chat_stream 循环每次迭代检查 _stop，命中时 flush 残余 buffer + emit chat_aborted + return。
- chat_page.py 加 _cancel_chat + _on_chat_aborted 方法：_cancel_chat 调 worker.stop() + 同步立即调 _on_chat_aborted() 清理 UI（不等后台信号）；_on_chat_aborted 幂等保护（pending_bubble 已 None 时 no-op）+ setText 同步刷新气泡显示 [已取消] 后缀。
- main_window.py 加 1 行 connect：toolbar.cancel_requested.connect(chat_page._cancel_chat)。
- stop-active.svg 图标：外圈 circle(12,12,10) 描边（继承顶层 stroke=currentColor）+ 内部 rect(8.5,8.5,7,7, fill=#EF4444, stroke=none) 红色方块。
- 取消按钮位置：从 _build_actions（左侧 about 前）移到 _build_model_group 里 model_combo 之后。新位置顺序：[新建][清空][关于] | [模型:][下拉框][取消][刷新][释放]。

### 取舍

- 唤醒阶段取消无反应的根因：ChatWorker.run() 在 chat_stream 阻塞等首个 chunk 时无法检查 _stop 标志。解法：UI 同步立即清理（不等后台信号），后台 stop() 仅设标志让线程自然早退。chat_aborted 信号到达时 _on_chat_aborted 幂等 no-op（pending 已 None）。这是"UI 不阻塞于后台线程"的通用模式。
- 气泡 [已取消] 后缀同步显示：原版本只 _messages.append 加后缀，气泡 text 没更新。改为先 setText 同步刷新气泡显示，再 _messages.append 入历史。
- 按钮位置移到下拉框右侧：符合用户视觉直觉——取消是针对当前选中模型的操作，与模型下拉框视觉关联比与"新建/清空"关联更强。

### 后果

- 接口预留型待办第二例落地（v0.0.8 ChatWorker.stop 占位 → v0.0.17 接通），证明 v0.0.8 接口预留策略的正确性。
- 落实 ADR-027 模块化原则的第四个执行案例（v0.0.14 重构 + v0.0.15 硬件监控独立窗口 + v0.0.16 新建清空按钮 + v0.0.17 取消按钮）。
- v0.0.8 ChatWorker.stop() 接口预留策略被验证，未来类似异步操作（如 ModelRefreshWorker/ModelReleaseWorker）如需取消可同模式扩展。

### 测试

- ruff check + format + mypy strict 全绿（43 源文件）
- pytest：156 通过（+4 新增：cancel_requested 信号 emit / chat_aborted 信号 / _on_chat_aborted 保留部分回复 / 取消按钮位置在 model_combo 之后），覆盖率 84.55%

## ADR-032：规则文件审查修订机制 + 设计规范纳入 L3 + 打包命令唯一来源（2026-06-22）

### 背景

项目规则文件（CLAUDE.md + memory/ 下 L3 文件）经多轮增量新增后，条例间出现冲突与冗余：
- 打包命令实体在 CLAUDE.md 第十七节与 安装包/README.md 两处重复，两份不一致时无权威源
- 设计规范 memory/设计规范/MASTER.md 实际已是 AI 工作记忆的一部分，但未在 CLAUDE.md 第二节 L3 清单登记
- manifest.json 作为 SVG 伴随元数据被第十八节"只存 SVG"字面规则误伤
- C 盘保护在第十七/二十节重复表述，错误追踪分线在第六/十三节重复表述，方案文件夹在第七/十六节重复表述
- 第二十一节模块化原则与第十九节指令自由度交叉时无明确优先级

### 决策

1. **规则文件审查修订机制**：当规则文件累积冲突时，派 3 个子 agent 并行审查（规则间冲突 / 规则与代码脱节 / 规则与项目实际状态偏差），汇总后分批修订。本次（2026-06-22）审查出 17 项冲突，分 6 批修订。
2. **打包命令唯一来源**：CLAUDE.md 第十七节删除命令实体，仅保留"必须严格从 安装包/README.md 复制粘贴"的引用指针。安装包/README.md 为唯一权威源。
3. **设计规范 MASTER.md 纳入 L3**：CLAUDE.md 第二节 L3 清单新增"设计系统规范 memory/设计规范/MASTER.md"。规范文件本身在头部注释标注"L3 完全权限"。
4. **manifest.json 例外条款**：第十八节新增"manifest.json 作为 SVG 伴随元数据例外允许"。
5. **去重**：第六/十六节改指针引用第七/十三节，第十二节改指针引用第五节，第十七/二十节 C 盘保护改引用第十一节，第十九节加指向第二十一节。
6. **规则审查触发条件**：当用户提出"规则文件冲突审查"请求，或 AI 主动发现规则与代码脱节时触发。

### 取舍

- **指针引用 vs 重复表述**：选指针引用——单一权威源避免分叉，但要求读者跟随指针跳读。权衡：规则文件本身是 AI 高频读取的工作记忆，跳读成本低于分叉修复成本。
- **派多个子 agent 并行 vs 单 agent 串行**：选并行——三视角互不干扰（冲突/脱节/偏差），汇总时再交叉。单 agent 串行容易陷入单一视角盲区。
- **不直接改 theme.py 既有模块样式**：设计规范补充发现 8 项脱节（如错误气泡颜色 #FEE2E2 与暗色基调冲突），但 theme.py 属 v0.0.1 既有模块，按模块化原则不动既有模块。仅把规范补到 MASTER.md 作为未来优化的待办标注，不立即改代码。

### 后果

- 规则文件从"累积冲突"状态恢复到"单一权威源 + 指针引用"状态，AI 读取时不再被矛盾条例误导。
- 设计规范 MASTER.md 正式成为 L3 工作记忆，未来 UI 元素新增时 AI 会主动读取并补录 Component Specs。
- manifest.json 不再被第十八节字面规则误伤，可作为 SVG 伴随元数据正常使用。
- 规则审查机制明文化，未来累积冲突时可按本机制再次触发。

### 测试

- ruff check + format + mypy strict 全绿
- pytest：156 通过，覆盖率 84.55%（本次仅改规则文件与文档，无源码改动）

## ADR-033：v0.0.18 编排层骨架（7 模块独立挂载 + 4 级压缩 + sqlite3 持久化）（2026-06-23）

### 背景

v0.0.17 之前 UI 直接调 OllamaClient.chat_stream，无编排层。市场调研完成 4 项目（claude code / openclaw / codex / opencode）拆解 + Q-agent 编排层架构 HTML 75KB 17 章后，启动 v0.0.18 编排层骨架开发。目标：支持 2000+ 轮对话，4 级压缩策略，零第三方依赖（用 Python 标准库 sqlite3），异步真摘要（SummaryWorker QThread + 独立小摘要模型 Qwen2.5-3B），标识符保留（第 3 级真摘要时强制保留文件路径/任务 ID/工具调用 ID/URL/哈希/模型名/函数名/错误码）。

### 决策

编排层 7 模块**独立挂载**（不动既有 core.py/planner.py，作 LegacyOrchestrator 向后兼容）：

1. **types.py** — 纯 dataclass：Role(str,Enum)/ToolCall/ToolResult/Message/TerminationReason(str,Enum)/TurnState/TurnResult/SessionData/CompactionRecord
...
5. **context.py** — ContextManager 4 级压缩：
   ...
   - L3 异步真摘要（SummaryWorker QThread + qwen2.5:3b 独立小摘要模型 + SUMMARY_SYSTEM_PROMPT 强制保留标识符）
   - L4 硬溢出终止
...
- **独立挂载 vs 重写 core.py**：选独立挂载——既有 core.py/planner.py 不动（符合 ADR-027 模块化原则），新 7 模块独立挂载，旧作 LegacyOrchestrator 向后兼容。cli.py + test_smoke.py 直接 `from q_agent.orchestrator.core import Orchestrator` 不受影响
- **sqlite3 vs 第三方 ORM**：选 sqlite3——Python 标准库零新依赖（符合 ADR-015），三表 + 两索引足够 2000+ 轮场景。子查询 `SELECT *` 不含 rowid 是陷阱，解法是显式 `SELECT rowid, ...`
- **4 级压缩 vs 单一截断**：选 4 级——L1 工具结果预算（最廉价）→ L2 旧消息截断（次廉价）→ L3 异步真摘要（最贵但保留语义）→ L4 硬溢出终止（兜底）。分级触发避免一上来就摘要
- **SummaryWorker QThread 骨架 vs 立即实化**：选骨架——v0.0.18 只验证信号触发 + stop 标志 + 占位输出格式，v0.0.19 实化真调 Ollama。避免骨架阶段引入 Ollama 依赖耦合
- **标识符保留指令**：SUMMARY_SYSTEM_PROMPT 强制保留文件路径/任务 ID/工具调用 ID/URL/哈希/模型名/函数名/错误码——避免真摘要后丢失关键引用导致 LLM 无法继续操作
- **整体覆写 vs 字段修改**：TurnState 用 dataclasses.replace 整体覆写——避免部分字段修改导致状态不一致

### 后果

- 编排层完全独立挂载，旧 core.py/planner.py 不动，符合 ADR-027 模块化原则第五个执行案例
- 4 级压缩策略支持 2000+ 轮对话，L1/L2 廉价截断避免频繁摘要，L3 异步真摘要不阻塞主循环，L4 兜底硬溢出
- sqlite3 零第三方依赖持久化，threading.Lock 保护连接，load_messages(limit) 支持分页加载 2000+ 轮
- SummaryWorker QThread 异步真摘要不阻塞主循环，summary_completed/summary_failed 信号回主线程合流
- 标识符保留指令确保真摘要后 LLM 仍能引用文件路径/任务 ID/工具调用 ID 等关键信息
- v0.0.18 骨架版 UI 看不到变化（UI 仍直接调 OllamaClient.chat_stream），v0.0.20 才接通 UI
- 测试 251 通过（+95 新增），87% 覆盖率，loop.py 76% 因异步摘要合流 + L4 硬溢出 + SummaryWorker 失败降级路径骨架阶段难测，待 v0.0.19 实化后补测

### 测试

- ruff check + format + mypy strict 全绿
- pytest：251 通过，覆盖率 87%（编排层平均 93.3%：types 100% / turn 98% / summary_worker 96% / dispatcher 95% / context 94% / persistence 94% / loop 76%）
- 7 个测试文件覆盖 10 步主循环各路径：自然停止 / 工具循环 / LLM 失败 / doom_loop / max_steps / 压缩触发 / on_step 回调 / on_compaction 回调 / 持久化 / user_message_appended_first
