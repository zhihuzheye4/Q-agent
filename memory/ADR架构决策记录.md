...
**取代**：无
**关联**：ADR-22（ChatWorker 流式批量刷新，v0.0.8 已留 stop() 占位）、ADR-27（贴纸式开发原则）、ADR-30（接口预留型待办首例 v0.0.8 status_callback → v0.0.16）
...
- 落实 ADR-027 贴纸式原则的第四个执行案例（v0.0.14 重构 + v0.0.15 硬件监控独立窗口 + v0.0.16 新建清空按钮 + v0.0.17 取消按钮）。
...
- **不直接改 theme.py 基座样式**：设计规范补充发现 8 项脱节（如错误气泡颜色 #FEE2E2 与暗色基调冲突），但 theme.py 属 v0.0.1 基座，按贴纸式原则不动基座。仅把规范补到 MASTER.md 作为未来优化的待办标注，不立即改代码。
